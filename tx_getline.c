...
ssize_t read_bytes, cmd_index = 0;
...
/* Read from the file descriptor fd into buff */
read_bytes = read(info->fd, &buff, BUFFER_SIZE - 1);
if (read_bytes == 0)
return (-1);
...
for (cmd_index = 0; cmd_input[cmd_index]; cmd_index++)
{
cmd_input[cmd_index] = str_dup(_strtok(cmd_index ? NULL : buff, "\n;"));
